...
class Replay
{
    ...
    public function SaveReplay()
    {
        ...
        #if sys
        File.saveContent(Main.getStoragePath() + "assets/replays/replay-" + PlayState.SONG.song + "-time" + Date.now().getTime() + ".kadeReplay", data);
        #end
    }
    ...
    {
        #if sys
        trace('loading ' + Main.getStoragePath() + 'assets/replays/' + path + ' replay...');
        try
        {
            var repl:ReplayJSON = cast Json.parse(File.getContent(Main.getStoragePath() + "assets/replays/" + path));
            replay = repl;
        }
        catch(e)
        {
